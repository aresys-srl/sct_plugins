...
from eo_products.saocom.reader import open_product, read_channel_data, read_channel_metadata
...
class SAOCOMDopplerPolynomial:
    """SAOCOM doppler polynomial wrapper compliant with PERSEO-quality Coordinate Conversion Function protocol"""
...
class SAOCOMProductManager:
    """SCTInputProduct protocol compliant SAOCOM wrapper"""
...
class SAOCOMChannelManager:
    ...
    def __init__(
        ...
        if self._channel.pulse is not None:
            self._signal_pulse_rate = self._channel.pulse.bandwidth_hz / self._channel.pulse.length_s
        else:
        ...
        self._doppler_centroid_poly = SAOCOMDopplerPolynomial(evaluator=self._channel.doppler_centroid_poly)
        self._doppler_rate_poly = SAOCOMDopplerPolynomial(evaluator=self._channel.doppler_rate_poly)
    ...
    def read_data(
        ...
        data = read_channel_data(
            ...
            binary_ordering_mode=self._channel.binary_ordering_mode,
            header_offset=self._channel.header_offset,
            row_prefix=self._channel.row_prefix,
        ).T
