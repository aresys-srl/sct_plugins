...
from sct_aresys_reader.reader.io.parsing.metadata_parsing import parse_metadata, serialize_metadata
...
def read_raster_as_memmap_with_raster_info(
    ...
    raster_info: metadata.RasterInfo,
) -> Iterator[MemmappedNDArray]:
    """Creates and manages the lifecycle of a read-only memmap to a raster file using information from a RasterInfo
    metadata object.
...
def read_raster_as_memmap(
    ...
    num_of_samples: int,
    num_of_lines: int,
    data_type: str | metadata.ECellType = metadata.ECellType.float32,
    binary_ordering_mode: str | metadata.EByteOrder = metadata.EByteOrder.le,
    header_offset: int = 0,
    row_prefix: int = 0,
) -> Iterator[MemmappedNDArray]:
    ...
    num_of_lines : int
        number of lines to be read
    data_type : str | metadata.ECellType, optional
        data type corresponding to the raster itself, by default metadata.ECellType.float32
    binary_ordering_mode : str | metadata.EByteOrder, optional
        binary ordering mode corresponding to the raster itself, by default metadata.EByteOrder.le
    header_offset : int, optional
        header offset of the raster file, by default 0
    row_prefix : int, optional
        row prefix of the raster file, by default 0; if a prefix larger than zero is provided, an exception will be
        raised as numpy memmaps do not support row prefixes
    ...
    if row_prefix > 0:
        raise NotImplementedError("Non-null row prefix is not supported in memmap read.")

    if data_type.value not in data_type_dict:
        raise InvalidDataType(f"Unknown data type id: {data_type.value}")
    data_type_numpy_value = data_type_dict[data_type.value]

    file_data_type = np.dtype(byte_order_dict[binary_ordering_mode.value] + data_type_numpy_value)

    memmap_shape = (num_of_lines, num_of_samples)

    with (
        open(raster_file_name, "rb") as f,
        mmap.mmap(
            f.fileno(),
            length=0,
        ...
        yield np.ndarray(memmap_shape, dtype=file_data_type, offset=header_offset, buffer=memmap_obj)
