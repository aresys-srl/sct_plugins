...
def translate_burst_info_from_model(
    ...
    for index, (burst, lines) in enumerate(zip(info.burst, lines_per_burst, strict=False)):
...
def translate_state_vectors_from_model(
    ...
    for index, (pos, vel) in enumerate(zip(state_vectors.p_sv_m.val, state_vectors.v_sv_m_os.val, strict=False)):
...
def translate_state_vectors_to_model(
    ...
    for index, (pos, vel) in enumerate(zip(state_vectors.position_vector, state_vectors.velocity_vector, strict=False)):
    ...
    return metadata_models.PolyType(
        pol=metadata_models.PolyType.Pol(
            [
                ...
                for index, (coeff, unit) in enumerate(zip(poly.coefficients, poly.get_units(), strict=False))
            ]
        ),
