...
import sct_aresys_reader.reader.io.io_support as io_utils
import sct_aresys_reader.reader.io.metadata as mtd
from sct_aresys_reader.reader.io.io_support import MemmappedNDArray
...
class PointSetProduct:
    ...
    @contextmanager
    def read_data_as_memmap(self) -> Iterator[tuple[CoordinatesMemmap, RCSMemmap]]:
        """Creates and manages the lifecycle of a memmap (read mode) to the Point Target Binary rasters.

        Suggested use: reading only a desired portion of data to memory, e.g. if the rasters are large.
        ...
        with ExitStack() as stack:
            for index, file in enumerate(self._coords_raster_files + self._rcs_raster_files):
                raw_memmaps.append(
                    stack.enter_context(
                        io_utils.read_raster_as_memmap_with_raster_info(
                            raster_file=file, raster_info=self._raster_infos[index]
                        )
                    )
                )
