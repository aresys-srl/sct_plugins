...
class RADARSAT2ChannelManager:
    ...
    def __init__(self, channel_name: str, channel_raster_path: Path, channel_metadata: RADARSATChannelMetadata) -> None:
        ...
        self._doppler_centroid_poly = RADARSAT2DopplerPolynomial(evaluator=self._channel.doppler_centroid_poly)
    ...
    def pixel_to_times_conversion(
        ...
        if self._lines_order_flip_flag:
            azimuth_index = self._channel.raster_info.lines.length - azimuth_index
    ...
    def times_to_pixel_conversion(
        ...
        # TODO: check this
        if self._samples_order_flip_flag:
            rng_idx = self._channel.raster_info.samples.length - rng_idx
        ...
        # TODO: check this
        if self._lines_order_flip_flag:
            azmth_idx = self._channel.raster_info.lines.length - azmth_idx
