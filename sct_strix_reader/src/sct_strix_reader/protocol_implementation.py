...
"""StriX format reader protocol-compliant wrapper for PERSEO-quality."""
...
from eo_products.strix.reader import open_product, read_channel_data, read_channel_metadata
from numpy.typing import ArrayLike
from perseo_core.geometry import (
...
class StriXDopplerPolynomial:
    """StriX doppler polynomial wrapper compliant with PERSEO-quality Coordinate Conversion Function protocol"""
...
class StriXProductManager:
    """SCTInputProduct protocol compliant StriX wrapper"""
    ...
    def get_channel_data(self, channel_id: str) -> StriXChannelManager:
        ...
        """
        return StriXChannelManager(
            channel_files=self._product.get_files_from_channel_name(channel_name=channel_id),
            channel_name=channel_id,
        )


class StriXChannelManager:
    """PERSEO-quality ChannelData protocol compliant StriX channel wrapper"""

    def __init__(
        self,
        channel_files: list[Path] | Path,
        channel_name: str,
    ) -> None:
        ...
        ----------
        channel_files : list[Path] | Path
            file(s) corresponding to the selected channel
        channel_name : str
            name of the current channel
        """

        self._channel_id = channel_name
        if isinstance(channel_files, list):
            # GRD case
            self._channel = read_channel_metadata(file_path=channel_files[0], channel_id=channel_name)
            self._raster_file = channel_files[1]
        else:
            # SLC case
            self._channel = read_channel_metadata(file_path=channel_files, channel_id=channel_name)
            self._raster_file = channel_files
        ...
        self._image_type = SARImageType[self._channel.general_info.processing_level.name]
        ...
        self._doppler_centroid_poly = StriXDopplerPolynomial(evaluator=self._channel.doppler_centroid_poly)
        self._doppler_rate_poly = StriXDopplerPolynomial(evaluator=self._channel.doppler_rate_poly)
    ...
    @property
    def doppler_centroid(self) -> StriXDopplerPolynomial:
        """Channel doppler centroid polynomial wrapper"""
        return self._doppler_centroid_poly

    @property
    def doppler_rate(self) -> StriXDopplerPolynomial:
    ...
    def get_location_data(self, azimuth_time: PreciseDateTime, range_time: float) -> LocationData:
        """Generating a LocationData object containing data and info derived from the current StriXChannelManager
    ...
    def read_data(
        ...
        data = read_channel_data(
            ...
            orbit_direction=self._channel.general_info.orbit_direction,
            looking_side=self._channel.dataset_info.side_looking,
